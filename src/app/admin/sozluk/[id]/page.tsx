...
export default function EditTermPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const fetchTerm = async () => {
    try {
      const res = await fetch(`/api/glossary/${unwrappedParams.id}`);
      if (res.ok) {
        const data = await res.json();
        setFormData({
          slug: data.slug || '',
          kavramTr: data.kavramTr || '',
          kavramEn: data.kavramEn || '',
          harfTr: data.harfTr || '',
          harfEn: data.harfEn || '',
          tanimTr: data.tanimTr || '',
          tanimEn: data.tanimEn || '',
          dayanakTr: data.dayanakTr || '',
          dayanakEn: data.dayanakEn || '',
          kategoriTr: data.kategoriTr || '',
          kategoriEn: data.kategoriEn || ''
        });
      } else {
        setError('Terim bulunamadı.');
      }
    } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const data = await res.json();
        setError(data.error || 'Terim güncellenirken bir hata oluştu.');
      }
    } catch (err) {
      setError('Bağlantı hatası oluştu.');
    } finally {
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      <div className="flex items-center gap-4 border-b border-border-primary pb-4">
        ...
        <h1 className="text-3xl font-serif font-bold text-text-primary">Terimi Düzenle</h1>
      </div>
